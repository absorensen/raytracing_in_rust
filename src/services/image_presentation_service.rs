...
pub fn present_image(config: &RenderConfig, image: Vec<ColorRGB>) {
    let window_buffer: Vec<u32> = convert_image_for_presentation(config, &image);
...
fn convert_image_for_presentation(config: &RenderConfig, image: &Vec<ColorRGB>) -> Vec<u32> {
    let black: ColorRGB = ColorRGB::black();
    let mut final_image: Vec<ColorRGB> = vec![black; image.len()];

    for row_index in 0..config.image_height {
